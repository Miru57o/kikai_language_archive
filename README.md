...
* **詳細なフィルタリング**: 集落、ファイルの種類、キーワードなどで柔軟にデータを絞り込めます
* **話者別・集落別閲覧**: 特定の話者や集落に紐づく記録を一覧表示できます
* **言語使用頻度の記録**: 各記録における言語使用頻度(日常的/よく使用/たまに使用/ほとんど使用しない)を記録できます

### 地図機能
* **インタラクティブな地図表示**: 喜界島の地図上で話者と地理環境データをビジュアル表示
* **年代フィルタリング**: 収録年でデータを絞り込んで表示可能
...
**注意事項:**
- `SECRET_KEY` には強力なランダム文字列を設定してください
...
2. 新しいWeb Serviceを作成し、このリポジトリを接続
3. 環境変数を設定(SECRET_KEY, DATABASE_URL, SUPABASE_URLなど)
4. Build Command: `./build.sh`
5. Start Command: `gunicorn kikai_archive_project.wsgi:application`

詳細は[Renderのドキュメント](https://render.com/docs)を参照してください。